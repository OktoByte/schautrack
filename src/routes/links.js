...
function jsonResponse(res, status, message, httpCode) {
...
router.post('/settings/link/request', requireLogin, csrfProtection, async (req, res) => {
  ...
  if (!emailRaw) {
    return jsonResponse(res, 'error', 'Email is required.');
  }
  ...
  try {
    ...
    const target = rows[0];
    if (!target) {
      return jsonResponse(res, 'error', 'No account found for that email.', 404);
    }
    const currentId = toInt(req.currentUser.id);
    const targetId = toInt(target.id);
    if (currentId === null || targetId === null) {
      return jsonResponse(res, 'error', 'Could not send link request.', 500);
    }
    if (targetId === currentId) {
      return jsonResponse(res, 'error', 'You cannot link to your own account.');
    }

    const existing = await getLinkBetween(currentId, targetId);
    if (existing) {
      if (existing.status === 'accepted') {
        return jsonResponse(res, 'error', 'You are already linked with this account.', 409);
      } else if (existing.requester_id === req.currentUser.id) {
        return jsonResponse(res, 'error', 'Request already sent and pending approval.', 409);
      } else {
        return jsonResponse(res, 'error', 'They already sent you a request. Check incoming requests below.', 409);
      }
    }

    const myAccepted = await countAcceptedLinks(currentId);
    if (myAccepted >= MAX_LINKS) {
      return jsonResponse(res, 'error', `You already have ${MAX_LINKS} linked accounts.`, 409);
    }

    const targetAccepted = await countAcceptedLinks(targetId);
    if (targetAccepted >= MAX_LINKS) {
      return jsonResponse(res, 'error', 'The other account already reached the linking limit.', 409);
    }
...
router.post('/settings/link/respond', requireLogin, csrfProtection, async (req, res) => {
  ...
  if (Number.isNaN(requestId) || !['accept', 'decline'].includes(action)) {
    return jsonResponse(res, 'error', 'Invalid request.');
  }

  try {
    const currentId = toInt(req.currentUser.id);
    if (currentId === null) {
      return jsonResponse(res, 'error', 'Could not update request.', 500);
    }
    ...
    if (!request || request.target_id !== currentId) {
      return jsonResponse(res, 'error', 'Request not found.', 404);
    }
    ...
    if (action === 'accept') {
      ...
      try {
        ...
        if (requesterAccepted >= MAX_LINKS) {
          await client.query('ROLLBACK');
          return jsonResponse(res, 'error', 'The requester is already at the link limit.', 409);
        }
      ...
      return jsonResponse(res, 'success', 'Link request accepted.');
    } else {
      ...
      return jsonResponse(res, 'success', 'Request declined.');
    }
  } catch (err) {
...
router.post('/settings/link/remove', requireLogin, csrfProtection, async (req, res) => {
  ...
  if (Number.isNaN(linkId)) {
    return jsonResponse(res, 'error', 'Invalid link.');
  }

  try {
    const currentId = toInt(req.currentUser.id);
    if (currentId === null) {
      return jsonResponse(res, 'error', 'Could not update link.', 500);
    }
    const { rows } = await pool.query(
    ...
    if (rows.length === 0) {
      return jsonResponse(res, 'error', 'Link not found.', 404);
    }
    ...
    if (deleted.status === 'accepted') {
      ...
      return jsonResponse(res, 'success', 'Link removed.');
    } else {
      broadcastLinkChange(otherId, 'cancelled', { requestId: linkId });
      return jsonResponse(res, 'success', 'Request cancelled.');
    }
  } catch (err) {
