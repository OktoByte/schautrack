...
async function getUserById(id) {
  const { rows } = await pool.query(
    'SELECT id, email, daily_goal, totp_enabled, totp_secret, timezone, weight_unit, timezone_manual, preferred_ai_provider, ai_key, macros_enabled, macro_goals, goal_threshold FROM users WHERE id = $1',
    [id]
  );
  const user = rows[0];
  if (!user) return null;
